...
\begin{document}
  ...
  \section{Derivatives} \vspace{7mm}

  \framethis{Suppose $f: U \subset \C \ra \C$ is a complex function, and there exists
  a small open disc $D = \{z \in \C $ s.t. $ |z-z_0| < \epsilon\} \subset U$. The derivative of $f$
  at $z_0$ is $f'(z_0) = \lim_{h \to 0} \dis \frac{f(z_0+h) - f(z_0)}{h}$
  (if the limit exists).
  Other notation: $\dis \frac{df}{dz}(z_0)$ or $\dis \frac{dw}{dz}(z_0)$ if $w = f(z)$.}
\end{document}
